...
const LEVELS_EN = [{label:'Junior',years:'0–1 yrs'},{label:'Mid-Level',years:'1–3 yrs'},{label:'Mid-Senior',years:'3–5 yrs'},{label:'Senior',years:'5–8 yrs'},{label:'Senior+',years:'8+ yrs'},{label:'Manager',years:'Managing people'},{label:'Director',years:'Head of dept'},{label:'C-Suite',years:'CEO, CFO...'}];
const LEVELS_AR = [{label:'مبتدئ',years:'0–1 سنة'},{label:'متوسط',years:'1–3 سنوات'},{label:'متوسط+',years:'3–5 سنوات'},{label:'متقدم',years:'5–8 سنوات'},{label:'متقدم+',years:'8+ سنوات'},{label:'مدير',years:'يدير أشخاصاً'},{label:'مدير أول',years:'رئيس قسم'},{label:'الإدارة العليا',years:'رئيس تنفيذي...'}];
const LEVEL_MAP_AR_TO_EN = {'مبتدئ':'Junior','متوسط':'Mid-Level','متوسط+':'Mid-Senior','متقدم':'Senior','متقدم+':'Senior+','مدير':'Manager','مدير أول':'Director','الإدارة العليا':'C-Suite'};
const SENIORITY_AR = {'Junior':'مبتدئ','Mid-Level':'متوسط','Mid-Senior':'متوسط+','Senior':'متقدم','Senior+':'متقدم+','Manager':'مدير','Director':'مدير أول','C-Suite':'الإدارة العليا'};
...
const NATIONALITY_AR = {
  'GCC National':'مواطن خليجي','Arab Expat':'وافد عربي','Western Expat':'وافد أوروبي/أجنبي','Asian Expat':'وافد آسيوي',
  'Local National':'مواطن محلي','Arab (Other)':'عربي (دولة أخرى)'
};
const EDUCATION_AR = {
  "High School":'ثانوية عامة',"Bachelor's":'بكالوريوس',"Master's":'ماجستير',"PhD":'دكتوراه',"Professional Cert":'شهادة مهنية'
};
const JOB_SYNONYMS = [
...
export default function Explore() {
  ...
  const displayNationality = (n) => lang==='ar' && NATIONALITY_AR[n] ? NATIONALITY_AR[n] : n;
  const displayEducation = (e) => lang==='ar' && EDUCATION_AR[e] ? EDUCATION_AR[e] : e;
              ...
              <div key={d.id} className="explore-card" style={{background:'#13131f',border:'1px solid #1e1e2e',borderRadius:'16px',padding:'20px',transition:'border-color 0.2s'}}
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'12px',marginBottom:'12px'}}>
                  ...
                  <div style={{textAlign:'right',flexShrink:0}}>
                    ...
                    {d.bonus && <div style={{color:'#10b981',fontSize:'11px',marginTop:'2px'}}>+{displayCurrency(d.currency)} {Number(d.bonus).toLocaleString()} {lang==='ar'?'بونص سنوي':'bonus/yr'}</div>}
                  </div>
                </div>

                <div style={{display:'flex',flexWrap:'wrap',gap:'6px',marginBottom:'12px'}}>
                  {d.experience && tag(`⏱ ${d.experience} ${lang==='ar'?'سنوات':'yrs'}`,'#606070')}
                  {d.education && tag(`🎓 ${displayEducation(d.education)}`,'#606070')}
                  {d.nationality_type && tag(`🌍 ${displayNationality(d.nationality_type)}`,'#3b82f6')}
